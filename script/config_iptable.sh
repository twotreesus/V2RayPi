...
iptables -t mangle -A V2RAY -d "$LAN_CIDR" -p tcp -j RETURN # 直连局域网，避免 V2Ray 无法启动时无法连网关的 SSH，如果你配置的是其他网段（如 10.x.x.x 等），则修改成自己的
iptables -t mangle -A V2RAY -d "$LAN_CIDR" -p udp ! --dport 53 -j RETURN # 直连局域网，53 端口除外（因为要使用 V2Ray 的 DNS）
...
iptables -t mangle -A V2RAY_MASK -d "$LAN_CIDR" -p tcp -j RETURN # 直连局域网
iptables -t mangle -A V2RAY_MASK -d "$LAN_CIDR" -p udp ! --dport 53 -j RETURN # 直连局域网，53 端口除外（因为要使用 V2Ray 的 DNS）
